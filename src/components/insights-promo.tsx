...
export type InsightsPromoEditTarget = "newsletter" | "roster";
...
export function InsightsPromo({
  ...
  editing?: boolean;
  selected?: InsightsPromoEditTarget | null;
  onSelect?: (target: InsightsPromoEditTarget) => void;
} = {}) {
    ...
    <div className="mt-10 grid gap-4 md:mt-12 md:grid-cols-2 md:gap-5">
      {showNewsletter ? (
        <EditableHit
          ...
          onSelect={() => onSelect?.("newsletter")}
          label="Newsletter card"
          block
          ringOffset="ring-offset-cream"
        >
          <div className="relative flex min-h-[13.75rem] flex-col justify-between overflow-hidden rounded-sm bg-cream-dark px-6 py-7 md:px-8 md:py-8">
            <div>
              {newsletter.eyebrow.trim() ? (
                ...
                <p className="text-[0.65rem] font-medium tracking-[0.16em] text-charcoal/30 uppercase">
                  Newsletter
                </p>
              ) : null}
              {newsletter.headline.trim() ? (
                ...
                <h2 className="mt-3 max-w-[18ch] font-display text-[1.55rem] leading-[1.1] tracking-tight text-charcoal/35 sm:text-[1.7rem] md:text-[1.85rem]">
                  Headline
                </h2>
              ) : null}
              {newsletter.description.trim() ? (
                ...
                <p className="mt-3 max-w-sm text-[0.9rem] leading-relaxed text-charcoal/40">
                  Description…
                </p>
              ) : null}
            </div>
            ...
            {editing ? (
              <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:items-stretch">
                <div className="min-h-11 w-full flex-1 rounded-sm border border-forest/30 bg-[#FBF8F5] px-3.5 text-[0.9rem] text-charcoal/40 sm:flex sm:items-center">
                  {newsletter.emailPlaceholder || "Work email"}
                </div>
                <div className="min-h-11 shrink-0 rounded-sm bg-forest px-5 text-[0.8125rem] font-medium text-cream sm:flex sm:items-center sm:px-6">
      ...
      {showRoster ? (
        ...
        >
          {editing ? (
            <div className="group relative flex min-h-[13.75rem] overflow-hidden rounded-sm bg-forest px-6 py-7 md:px-8 md:py-8">
              <PatternField
                color={CREAM_RGB}
              ...
              <div className="relative z-2 flex flex-1 flex-col justify-between">
                <div>
                  {roster.eyebrow.trim() ? (
                    ...
                    <p className="text-[0.65rem] font-medium tracking-[0.16em] text-cream/35 uppercase">
                      Roster
                    </p>
                  )}
                  {roster.headline.trim() ? (
                    ...
                    <h2 className="mt-3 max-w-[16ch] font-display text-[1.55rem] leading-[1.1] tracking-tight text-cream/40 sm:text-[1.7rem] md:text-[1.85rem]">
                      Headline
                    </h2>
                  )}
                  {roster.description.trim() ? (
                    ...
                    <p className="mt-3 max-w-sm text-[0.9rem] leading-relaxed text-cream/45">
                      Description…
                    </p>
                  )}
                </div>
